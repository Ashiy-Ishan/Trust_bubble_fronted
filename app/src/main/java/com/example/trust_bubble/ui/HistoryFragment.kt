...
class HistoryFragment : Fragment() {
    ...
    private fun setupRecyclerView() {
        historyAdapter = HistoryAdapter { analysisItem ->
            // This code runs when a user clicks an item
            val intent = Intent(requireContext(), DecisionTreeActivity::class.java).apply {
                // The key is "decision_tree_json"
                // The value is the decisionPath object converted to a JSON string
                putExtra("decision_tree_json", Gson().toJson(analysisItem.decisionPath))
            }
            ...
            try {
                ...
                withContext(Dispatchers.Main) {
                    if (response.isSuccessful) {
                        ...
                        binding.historyRecyclerView.visibility = if (historyList.isEmpty()) View.GONE else View.VISIBLE
                    } else {
